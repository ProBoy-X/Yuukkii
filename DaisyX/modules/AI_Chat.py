...
async def hmm(_, message):
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            ...
            return
        await lel.edit(
            f"Yuki AI Successfully Added For Users In The Chat {message.chat.id}"
        )
        ...
        if not Escobar:
            ...
            return
        await lel.edit(
            f"Yuki AI Successfully Deactivated For Users In The Chat {message.chat.id}"
        )
...
@daisyx.on_message(
    filters.regex("Yuki|Yuki|Yuki|Yuki|Yuki")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
<b> Chatbot </b>
YUKI AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
...
<b> Assistant </b>
 - /ask [question]: Ask question from Yuki
